...
__model_data_dir = os.path.dirname(os.path.realpath(__file__)) + "/model_data/"
...
def core_zeng19_rock(mcore: float, **kwargs) -> float:
    ...
    if __model_data_cache_zeng19['rock'] is None:
        data = np.loadtxt(__model_data_dir + 'zeng19/core_zeng19_rock.csv', delimiter = ',')
        masses, radii = data.T
        __model_data_cache_zeng19['rock'] = interp1d(x = masses, y = radii,
            kind = 'cubic', fill_value="extrapolate", assume_sorted = True)

    model = __model_data_cache_zeng19['rock']
...
def core_zeng19_iron(mcore: float, **kwargs) -> float:
    ...
    if __model_data_cache_zeng19['iron'] is None:
        data = np.loadtxt(__model_data_dir + 'zeng19/core_zeng19_iron.csv', delimiter = ',')
        masses, radii = data.T
        __model_data_cache_zeng19['iron'] = interp1d(x = masses, y = radii,
            kind = 'cubic', fill_value="extrapolate", assume_sorted = True)

    model = __model_data_cache_zeng19['iron']
    return float(model(mcore))
...
def core_zeng19_water(mcore :float, lbol :float, sep :float, albedo :float = 0.2, **kwargs):
    ...
    if __model_data_cache_zeng19['water'] is None:
        # Load model sequences
        temp = 300, 500, 700, 1000
        sequences = [ np.loadtxt(__model_data_dir + f"zeng19/core_zeng19_water_{t}K.csv", delimiter = ',') for t in temp ]
        models    = [
        ...
        # 2D interpolation
        __model_data_cache_zeng19['water'] = RegularGridInterpolator(
    ...
    model = __model_data_cache_zeng19['water']
...
def core_zeng19_water_rock(mcore :float, lbol :float, sep :float, albedo :float = 0.2, **kwargs):
    ...
    # Load cache of model sequences interpolating equilibrium temperature
    if __model_data_cache_zeng19['water_rock'] is None:
        # Load model sequences
        temp = 300, 500, 700, 1000
        sequences = [ np.loadtxt(__model_data_dir + f"zeng19/core_zeng19_water_rock_{t}K.csv", delimiter = ',') for t in temp ]
        models    = [
            interp1d(x = seq.T[0], y = seq.T[1],
                kind = 'cubic', fill_value="extrapolate", assume_sorted = True)
            for seq in sequences
        ...
        # 2D interpolation
        __model_data_cache_zeng19['water_rock'] = RegularGridInterpolator(
            points = (temp, regular_masses),
            values = regular_radii,
            method = 'slinear',
            bounds_error = False,
    ...
    model = __model_data_cache_zeng19['water_rock']
    return float(model([teq, mcore]))
